...
def lu(u: np.array, x: np.linspace, t: np.linspace,i, k, h):
    return (u[i + 1, k] - 2 * u[i, k] + u[i - 1, k]) / pow(h, 2)
...
def solve(h, tau):
    ...
    for k in range(1, n_t - 1):
        for i in range(1, n_x - 1):
            U[i, k + 1] = 2 * U[i, k] - U[i, k - 1] + tau ** 2 * (lu(U, xs, ts, i, k, h) + f(xs[i], ts[k]))
        U[0, k + 1] = (2 * h * alpha(ts[k + 1]) - U[2, k + 1] + 4 * U[1, k + 1]) / (2 * h + 3)
...
[xs, ts, U] = solve(0.25, 0.25) #Измение шага
